    ...
    <div>
        <section class="todo-wrapper">
            ...
            <div v-if="todo.pending.value.length > 0">
                <p class="status busy">
                    현재 할 일이 {{ todo.pending.value.length }}가지
                    있습니다.<span v-if="todo.pending.value.length > 1"></span>
                </p>
                <transition-group name="todo-item" tag="ul" class="todo-list">
                    <li
                        v-for="item in todo.pending.value"
            ...
            <div
                v-if="
                    todo.completed.value.length > 0 && todo.value.showComplete
                "
            >
                <p class="status">
                    완료된 사항: {{ todo.completedPercentage.value }}
                </p>
                <transition-group
                    name="todo-item"
                    tag="ul"
                ...
                >
                    <li
                        v-for="item in todo.completed.value"
                        v-bind:key="item.title"
                    >
                        <input
                            class="todo-checkbox"
            ...
            <div class="control-buttons">
                ...
                >
                    <span v-if="!todo.value.showComplete"
                        >완료한 사항 보이기</span
                    ><span v-else>완료한 사항 감추기</span>
                </div>
                <div
                    class="btn btn-secondary"
                    v-if="todo.value.todoList.length > 0"
                    @click="todo.clearAll()"
                >
                    알림 다 지우기
                </div>
